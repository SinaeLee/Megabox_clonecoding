...
def info(request, movie_id) :
    ...
    # list = []
    # for row in reader:
    #     list.append(test(movie_type=row[0], movie_title=row[1], movie_title_eng=row[2],
    ...
    # test.objects.bulk_create(list) # 이런식으로 리스트를 한번에 넣는다.
    movie = test.objects.get(id=movie_id)
    content = {'movie': movie}
